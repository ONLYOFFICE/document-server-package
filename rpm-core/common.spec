...
if [ ! -e ${NGINX_CONFD_DIR}/ds.conf ]; then
  %__ln_s \
    $CONF_DIR/nginx/ds.conf \
    ${NGINX_CONFD_DIR}/ds.conf
fi
...
NGINX_CONFD="/etc/nginx/conf.d"
CONF_DIR=%{_sysconfdir}/%{_ds_prefix}
HOME_DIR=%{_localstatedir}/www/%{_ds_prefix}

# Restore dse.conf for uninstall
if [ -h ${CONF_DIR}/dse/dse.conf ]; then
  if [ ! -h ${NGINX_CONFD}/dse.conf ] && [ -f $HOME_DIR-example/example ]; then
