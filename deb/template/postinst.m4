...
',)dnl
	...
	if [ ! -f $LOCAL_CONFIG ] && [ -z $JWT_SECRET ]; then
		JWT_MESSAGE="Since v7.2.0 we've enabled JWT with random key by default. You can lookup this key in ${LOCAL_CONFIG} in services.CoAuthoring.secret param or enabled your custom JWT key with debconf template M4_ONLYOFFICE_VALUE/jwt-secret"
		...
		JWT_MESSAGE="You have JWT disabled. We recommend enabling JWT with debconf template M4_ONLYOFFICE_VALUE/jwt-enabled and configure your custom JWT key with M4_ONLYOFFICE_VALUE/jwt-secret"
	fi
}
