...
sed "/user=/s/onlyoffice/root/" -i /etc/supervisor/conf.d/ExampleService.conf
...
mkdir -p /var/log/onlyoffice/documentserver/ExampleService/
...
# Disable cache
sed 's/{{DATE}}/'$(date +%F-%H-%M)'/'  -i ${NGINX_ONLYOFFICE_PATH}
